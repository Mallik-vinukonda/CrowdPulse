...
import { getCredits, getMyIssues } from "../utils/api";
...
export default function Dashboard() {
	...
	const fetchDashboard = async () => {
		...
		setError("");
		try {
			const [creditData, issueData] = await Promise.all([
				getCredits(),
				getMyIssues(),
			]);
			...
			<div className="max-w-5xl mx-auto space-y-8">
				...
				<div className="flex justify-end mb-4">
					<button
						...
						className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded shadow hover:bg-blue-700 font-semibold"
					>
				...
				<div className="grid md:grid-cols-2 gap-8">
					<div className="bg-white p-8 rounded-2xl shadow-lg border border-blue-100 flex flex-col items-center">
						...
						<p className="text-5xl text-blue-700 font-extrabold mt-4 mb-2">
							{loading ? (
						...
						<span className="text-sm text-gray-400">
							Earn more by reporting issues!
						</span>
					</div>
					<div className="bg-white p-8 rounded-2xl shadow-lg border border-blue-100">
						<h3 className="text-lg font-semibold text-gray-700 flex items-center justify-between">
							Latest Submissions
							<span className="ml-2 bg-blue-100 text-blue-700 px-2 py-1 rounded text-xs font-bold">
								{issues.length} total
							</span>
						</h3>
						{loading ? (
