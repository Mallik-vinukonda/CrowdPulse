...
export async function submitIssue(formData) {
	const { data } = await axios.post(`${API_URL}/api/issue`, formData);
	return data;
}

export async function getMyIssues() {
	const { data } = await axios.get(`${API_URL}/api/issue/mine`);
	return data;
}
...
export async function redeemCredits(credits, payoutMethod) {
	const { data } = await axios.post(`${API_URL}/api/credit/redeem`, {
		credits,
		payoutMethod,
	});
	return data;
...
export async function unlockCourse(id) {
	const { data } = await axios.post(`${API_URL}/api/course/${id}/unlock`);
	return data;
}

export async function getMyCourses() {
	const { data } = await axios.get(`${API_URL}/api/course/mine`);
	return data;
}

export async function getPendingIssues() {
	const { data } = await axios.get(`${API_URL}/api/issue?status=pending`);
	return data;
}

export async function reviewIssue(id, status) {
	const { data } = await axios.post(`${API_URL}/api/issue/${id}/review`, {
		status,
	});
	return data;
}
